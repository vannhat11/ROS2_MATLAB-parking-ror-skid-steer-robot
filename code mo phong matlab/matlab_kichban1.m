...
%% 2. THIẾT LẬP CÁC BIÊN TƯỜNG TĨNH BAO QUANH SA BÀN (Đã xóa vật cản)
% Các biên tường tĩnh bao quanh khu vực sa bàn (Chuyển đổi lại chỉ số index)
obs(1).b = [4.0; 3.0; 4.0; -3.0];
obs(2).b = [1.7; 3.0; 0.8; 0.0];
obs(3).b = [4.0; -2.3; 0.8; 0.0];
M = length(obs);

% Vị trí tư thế mục tiêu ô đỗ xe cuối cùng (Target Pose)
WP_Park = [2.0; 0.4; pi/2];

%% === THUẬT TOÁN NHẬN DẠNG: KIỂM TRA CHIỀU RỘNG BÃI ĐỖ ===
is_spot_valid = true;

% 1. Kiểm tra chiều rộng thực tế của bãi đỗ (Giữa tường tĩnh số 2 và số 3)
wall_left_x = obs(2).b(1);
wall_right_x = -obs(3).b(2);
...
% Khai báo các biến đối ngẫu nhân tử Lagrange phục vụ ràng buộc OBCA né biên tường
...
for k = 1:N
    ...
    for m = 1:M
        ...
        % Ràng buộc phi tuyến khoảng cách an toàn OBCA với tường bao sa bàn
...
% Đã xóa phần vẽ patch vật cản đen hình học thực tế
h_obs_black = plot(NaN, NaN, 'k', 'DisplayName', 'Obstacles'); % Giữ handle trống để không lỗi legend
...
for t = 1:T_sim
    if current_phase == 1
        ...
        for i = 1:size(candidates, 1)
            ...
            % Đã loại bỏ hoàn toàn kiểm tra khoảng cách d_obs1 và d_obs2 với vật cản
            if Pi(2) < 0.95 || Pi(2) > 2.85, continue; end
        ...
        % Đánh giá khoảng cách Euclid thực tế để đưa ra quyết định chuyển giai đoạn
        d_k = norm(x_curr(1:2) - WP_Park(1:2)); 
        if d_k <= d_sw && x_curr(1) >= 2.2
            if is_spot_valid
                current_phase = 2; 
            else
                current_phase = 3; 
